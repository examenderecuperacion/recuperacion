...
export const agregarPago =  (req, res)=>{
    const {id, pago} = req.body;
    try {
        const respuesta =  pool.query(`CALL SP_AGREGAR_PAGO ("${id}" , "${pago})"`)
        res.json({"respuesta": "producto agregado correctamente"})
    } catch (error) {
        res.json({"error": "producto no fue agregado"})
    }
}

export const modificarPago =  (req, res)=>{
    const {id, pago} = req.body;
    try {
        const respuesta =  pool.query(`CALL SP_MODIFICAR_PAGO ("${id}" , "${pago}")`)
